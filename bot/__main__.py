...
def start(update, context):
    buttons = ButtonMaker()
    buttons.buildbutton("owner", "https://t.me/peculiar5")
    buttons.buildbutton("Group", "https://t.me/+cj6D5OnSlz85NWRl")
...
'''
        ...
        sendMarkup('☣Not For Private Use!☢', context.bot, update.message, reply_markup)

def restart(update, context):
    restart_message = sendMessage("🥱Waking up, Love🥱", context.bot, update.message)
    if Interval:
        Interval[0].cancel()
        Interval.clear()
    alive.kill()
...
def main():
                ...
                for tag, links in data.items():
                     ...
                     for index, link in enumerate(links, start=1):
                         ...
                         if len(msg.encode()) > 4000:
                             if '🥰 Just woke up, Love! 🥰' in msg and cid == chat_id:
    ...
    if ospath.isfile(".restartmsg"):
        with open(".restartmsg") as f:
            chat_id, msg_id = map(int, f)
        bot.edit_message_text("🥰 Just woke up, Love! 🥰", chat_id, msg_id)
        osremove(".restartmsg")
    elif not notifier_dict and AUTHORIZED_CHATS:
        for id_ in AUTHORIZED_CHATS:
            try:
                bot.sendMessage(id_, "🥰 Just woke up, Love! 🥰", 'HTML')
            except Exception as e:
                LOGGER.error(e)
    ...
    bot.sendMessage(chat_id=-1001677879916, text="<b>❤I'm back, Love!❤</b>", parse_mode='HTMl')
